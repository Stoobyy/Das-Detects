    ...
    def _process_frames(self):
        ...
        if self._audio_recorder and self._call_active:
            ...
            if frame is not None:
                ...
                # Generate randomized confidence score (0-100)
                self._last_confidence = random.uniform(0.0, 100.0)
                
                # Classify based on confidence score
                if self._last_confidence < 50:
                    self._last_classification = "HUMAN"
                    conf_color = C["safe"]
                elif self._last_confidence < 70:
                    self._last_classification = "SUSPICIOUS"
                    conf_color = C["warn"]
                else:
                    self._last_classification = "AI"
                    conf_color = C["alert"]
                
                # Update UI with confidence and classification
                self.confidence_lbl.configure(
                    text=f"Confidence: {self._last_confidence:.1f}%",
                    text_color=conf_color
                )
                self.classification_lbl.configure(
                    text=f"Classification: {self._last_classification}",
                    text_color=conf_color
                )
                ...
                # Notify on AI detection
                if self._last_classification == "AI":
                    notify("AI Voice Detected", f"Confidence: {self._last_confidence:.1f}%")
                elif self._last_classification == "SUSPICIOUS":
                    notify("Suspicious Voice Detected", f"Confidence: {self._last_confidence:.1f}%")
